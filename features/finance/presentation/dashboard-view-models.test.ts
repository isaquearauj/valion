...
describe("dashboard view models", () => {
  ...
  it("detects due reminders only for active reminders", () => {
    expect(isReminderDue(reminder({ nextDueDate: "2026-01-10", status: "Ativo" }), "2026-01-10")).toBe(true)
    expect(isReminderDue(reminder({ nextDueDate: "2026-01-11", status: "Ativo" }), "2026-01-10")).toBe(false)
    expect(isReminderDue(reminder({ nextDueDate: "2026-01-01", status: "Pausado" }), "2026-01-10")).toBe(false)
  })

  it("calculates reminder progress and status priority", () => {
    expect(getReminderProgress(reminder({ totalInstallments: 0 }))).toBe(0)
    expect(getReminderProgress(reminder({ remainingInstallments: 2, totalInstallments: 4 }))).toBe(50)
    expect(getReminderProgress(reminder({ remainingInstallments: -1, totalInstallments: 4 }))).toBe(100)
  ...
  it("handles month key navigation across years", () => {
    ...
    expect(getAdjacentMonthKeys("2026-01")).toEqual(["2025-11", "2025-12", "2026-01", "2026-02", "2026-03"])
  })
  ...
  it("builds defaults for new and existing records", () => {
    ...
    expect(getIncomeDefaults(income)).toMatchObject({ amount: 500, frequency: "Semanal", type: "Freelance" })
    expect(getIncomeDefaults(null)).toMatchObject({ amount: "", frequency: "Mensal", type: "Salário" })
    expect(getReminderDefaults(null)).toMatchObject({ nextDueDate: "2026-01-08", status: "Ativo", type: "Recorrente" })
    expect(getExpenseDefaults(expense)).toMatchObject({ category: "Assinaturas", dueDay: 12, monthlyAmount: 90 })
    expect(getInvestmentDefaults(investment)).toMatchObject({ investedAmount: 100, month: "2026-02", plannedAmount: 200 })
    expect(getInvestmentDefaults(null).month).toBe("2026-01")
    expect(getGoalDefaults(goal({ targetDate: null }))).toMatchObject({ deadlineDate: "", deadlineEnabled: false })
    expect(getGoalContributionDefaults("", [goal({ id: "goal-a" })])).toMatchObject({ date: "2026-01-08", goalId: "goal-a" })
  ...
  it("summarizes goals and caps progress at 100%", () => {
    ...
    expect(getGoalProgress(goals[0], contributions.filter((item) => item.goalId === "g1"))).toEqual({
      currentAmount: 1200,
      percent: 100,
      remainingAmount: 0,
    })
    expect(isGoalCompleted(goals[0], contributions.filter((item) => item.goalId === "g1"))).toBe(true)
  ...
  it("builds goal timelines in chronological order", () => {
    expect(
      getGoalTimeline(goal({ targetAmount: 1000 }), [
        contribution({ amount: 200, createdAt: "2026-01-02T00:00:00.000Z", date: "2026-01-02", id: "c2" }),
        contribution({ amount: 100, createdAt: "2026-01-01T00:00:00.000Z", date: "2026-01-02", id: "c1" }),
        contribution({ amount: 50, createdAt: "2026-01-01T00:00:00.000Z", date: "2026-01-01", id: "c0" }),
      ])
    ).toMatchObject([
  ...
  it("sorts goals by completion, status, deadline and creation date", () => {
    const activeWithoutOwnContribution = goal({ createdAt: "2026-01-02T00:00:00.000Z", id: "active", targetDate: "2026-03-01" })
    ...
    const newerNoDeadline = goal({ createdAt: "2026-01-03T00:00:00.000Z", id: "newer", targetDate: null })
    const newerSameDeadline = goal({ createdAt: "2026-01-04T00:00:00.000Z", id: "newer-same", targetDate: "2026-03-01" })

    expect(
      sortGoals([completed, paused, newerNoDeadline, activeWithoutOwnContribution, newerSameDeadline], [
  ...
  it("normalizes goal form values with completion and deadline rules", () => {
    expect(
      normalizeGoalFormValues(
        { deadlineDate: "", deadlineEnabled: false, name: "Reserva", notes: "", status: "Ativa", targetAmount: 1000 },
        1000
      )
    ).toMatchObject({ status: "Concluída", targetDate: null })
    expect(
      normalizeGoalFormValues(
        { deadlineDate: "2026-12-31", deadlineEnabled: true, name: "Reserva", notes: "", status: "Pausada", targetAmount: 1000 },
        100
      )
