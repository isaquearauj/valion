...
`SUPABASE_DB_PASSWORD` e `SUPABASE_PROJECT_REF` desse environment, com
`contents: read`. O disparo manual existe apenas como fallback. Seed,
`supabase db reset --linked`, SQL arbitrário e `supabase db push` direto contra
produção são proibidos.
